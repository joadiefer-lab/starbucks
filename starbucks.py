...
def calcular_lca_mixto(file_path):
    df_input = load_data(file_path)
    MAX_LCA = 3000
    df_lca_sample = df_input.sample(min(MAX_LCA, len(df_input)), random_state=42).copy()
    ...
    df_train = prep_df(df_lca_sample)
    mm_data_train, mm_descriptor = get_mixed_descriptor(
        dataframe=df_train, gaussian=gaussianas,
        categorical=variables_categoricas, binary=binarias
    )
    ...
    # Predecir sobre dataset COMPLETO
    df_full = prep_df(df_input[variables_categoricas + gaussianas + binarias].copy())
    df_full_cat = df_input.copy()
    for col in variables_categoricas:
        df_full_cat[col] = df_full_cat[col].astype('category').cat.codes
    for col in binarias:
        df_full_cat[col] = df_full_cat[col].astype(int)
    mm_data_full, _ = get_mixed_descriptor(
        dataframe=df_full_cat[variables_categoricas + gaussianas + binarias],
        gaussian=gaussianas, categorical=variables_categoricas, binary=binarias
    )
    predicciones = model_final.predict(mm_data_full)
...
@st.cache_data
def generar_base_rfm(file_path):
    df_input = load_data(file_path)
...
with tab3:
    ...
    with st.spinner("Construyendo matriz transaccional por cliente..."):
        rfm_completo = generar_base_rfm(FILE_NAME)
...
# ── AÑADIR ESTO AL FINAL DEL WITH TAB4 ────────────────────────────────────
    ...
    df_rfm_copia = generar_base_rfm(FILE_NAME).copy()
